...
func (s summaryModel) View(width, maxHeight int) string {
	...
	panel += fmt.Sprintf("\n  %-18s %s    %s\n", sSubHeader.Render("Lines read:"), sValue.Render(pipeline.CommaFmt(s.linesRead)), sDim.Render("Data read:   "+pipeline.HumanSize(s.bytesRead)))
	panel += fmt.Sprintf("  %-18s %s    %s\n", sSubHeader.Render("Lines kept:"), sSuccess.Render(pipeline.CommaFmt(s.linesKept)), sDim.Render("Data written: "+pipeline.HumanSize(s.bytesWritten)))
	panel += fmt.Sprintf("  %-18s %s    %s\n", sSubHeader.Render("Lines dropped:"), sError.Render(pipeline.CommaFmt(s.linesDropped)), sDim.Render("Elapsed:      "+pipeline.FormatDuration(s.elapsed)))
	if throughput != "" {
